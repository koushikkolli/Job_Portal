...
$(document).ready(function() {
  ...
  $("#cpass").change(function() {
    ...
    if (m_pass.localeCompare(d) != 0) {
      ...
      document.getElementById("err_cpass").style.color = 'green';
      $('#err_cpass').text("match").show().fadeOut(2000);
      c_passwor_val = true;
    }
  });
});
...
function sign_up() {
  if (!email_val || !passwor_val || !name_val || !c_passwor_val) {
    $("#err")
    ...
    var pass = document.getElementById("password").value;
    var j_type = document.getElementById("type").value;
    var data = {
      name: name,
      email: mail,
      password: pass,
